  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {isOwner && task.status !== 'completed' && (
        <Card className="p-6">
          <h3 className="font-medium text-slate-800 mb-3">Submit your work</h3>
      ...
      {submissions?.length > 0 && (
        <Card className="p-6">
          <h3 className="font-medium text-slate-800 mb-4">Submissions</h3>
        ...
        <div className="space-y-3 mb-4 max-h-64 overflow-y-auto">
          {task.comments?.length ? task.comments.map((c, i) => (
            <div key={i} className="text-sm">
              ...
              <span className="text-slate-400 text-xs ml-2">{new Date(c.createdAt).toLocaleString()}</span>
